...
<script>
  ...
  firebase.analytics();
				...
				<div class="col-lg-12">
					<form name="userForm" ng-submit="submitForm(userForm.$valid)" novalidate>
						<div class="form-group" ng-class="{ 'has-error' : userForm.name.$invalid && !userForm.name.$pristine }">
							...
							<input type="text" name="name" class="form-control" id="nama"  ng-model="user.name" required>
							<small ng-show="userForm.name.$invalid && !userForm.name.$pristine" class="help-block text-danger">You
								name is
								required.
							</small>
							</p>
						...
						<div class="form-group" ng-class="{ 'has-error' : userForm.email.$invalid && !userForm.email.$pristine }">
							...
							<small ng-show="userForm.email.$invalid && !userForm.email.$pristine" class="help-block text-danger">Enter
								a valid
								email.
							</small>
						</div>
						<div class="form-group" ng-class="{ 'has-error' : userForm.person.$invalid && !userForm.person.$pristine }">
							<label for="person">Enter the Name of Recipe</label>
							<input type="text" name="person" class="form-control" id="menu"  ng-model="user.person">
							<small ng-show="userForm.person.$invalid && !userForm.person.$pristine"
								class="help-block text-danger">Enter
								a valid
								number
							</small>
						</div>
						<div class="form-group"
							...
							<textarea type="text" name="message" class="form-control" id=s"komentar"  ng-model="user.message" required></textarea>
							<small ng-show="userForm.message.$invalid && !userForm.message.$pristine"
								class="help-block text-danger">Your Message
								is required
							</small>
							</p>
						</div>
						<button type="submit" value="Save" onclick="save_user();" class="btn btn-red btn-lg btn-block text-uppercase">SEND MESSAGE</button>
					</form>
	...
	<!-- database -->
	<script>
	var databaseRef = firebase.database().ref('complainmenu/');
	function save_user(){
		var nama = document.getElemenById('nama').value;
		var email = document.getElemenById('email').value;
		var menu = document.getElemenById('menu').value;
		var komentar = document.getElemenById('komentar').value;
		
		var uid = firebase.database().ref().child('complainmenu').push().key;
		...
		var updates = {};
		updates['/complainmenu/' + uid] = data;
		firebase.database().ref().update(updates);
		
		alert('complain menu has been send!');
		reload_page();
	}
